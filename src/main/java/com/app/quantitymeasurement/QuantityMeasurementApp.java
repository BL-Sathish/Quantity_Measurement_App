...
public class QuantityMeasurementApp {
    ...
    public static void main(String[] args) {
        ...
        // UC16: Database Repository Demo
        System.out.println("\n=== UC16: Repository Persistence Demo ===");
        LOGGER.info("Repository type: " + repository.getClass().getSimpleName());
        System.out.println("Total saved operations: " + repository.getCount());
        repository.getAllMeasurements().stream().limit(10).forEach(System.out::println);
        if (repository.getCount() > 10) {
            System.out.println("... and " + (repository.getCount() - 10) + " more.");
        }

        // Print connection pool stats if using database
        if (connectionPool != null) {
            System.out.println("\n=== UC16: Connection Pool Statistics ===");
    ...
    /**
     * Initializes the application layers based on configuration.
     * Reads repository.type from application.properties to determine
     * whether to use cache or database repository.
     */
    private static void initializeApplication() {
        ApplicationConfig config = ApplicationConfig.getInstance();
        String repoType = config.getRepositoryType();

        LOGGER.info("Initializing application with repository type: " + repoType);

        if ("database".equalsIgnoreCase(repoType)) {
            try {
                // Load H2 driver
                Class.forName(config.getDbDriver());
                LOGGER.info("Database driver loaded: " + config.getDbDriver());
                ...
                connectionPool = new ConnectionPool(
                        ...
                        config.getPoolInitialSize(),
                        config.getPoolMaxSize()
                );
                LOGGER.info("Connection pool created: " + connectionPool.getPoolStats());
                ...
                LOGGER.log(Level.SEVERE, "Database driver not found. Falling back to cache repository.", e);
        ...
        LOGGER.info("Application layers initialized successfully.");
    }
    ...
    public static void demonstrateAddition(double val1, LengthUnit unit1, double val2, LengthUnit unit2) {
        ...
        System.out.printf("Input: add(%s, %s)%nOutput: %s%n%n", op1, op2, sum);
    }
    ...
    public static void demonstrateAddition(double val1, LengthUnit unit1, double val2, LengthUnit unit2, LengthUnit targetUnit) {
        ...
        System.out.printf("Input: add(%s, %s, %s)%nOutput: %s%n%n", op1, op2, targetUnit, sum);
    }
